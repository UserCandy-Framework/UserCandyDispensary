<?php
/**
* UserCandy Messages Models Plugin
*
...
class Messages extends Models {
        ...
        $data_check = $this->db->select("
          ...
          if($count > 0){
            $data_dr = array('date_read' => date('Y-m-d G:i:s'));
      			...
      			$this->db->update(PREFIX."plugin_messages",$data_dr,$where);
          }
			return $data;
		}else{
			return false;
  ...
  public function deleteMessageInbox($u_id, $msg_id){
    ...
    $query = $this->db->selectCount('SELECT * FROM '.PREFIX.'plugin_messages WHERE id = :msg_id AND to_userID = :u_id',
        array(':msg_id' => $msg_id, ':u_id' => $u_id));
    if($query == "1")
    {
      $data_del = $this->db->update(PREFIX.'plugin_messages', array('to_delete' => 'true'), array('id' => $msg_id));
      if($data_del > 0){
        return true;
      }else{
        return false;
  ...
  public function markReadMessageInbox($u_id, $msg_id){
    // Check to make sure message is not already marked as read
    $query = $this->db->selectCount('SELECT * FROM '.PREFIX.'plugin_messages WHERE id = :msg_id AND to_userID = :u_id AND date_read IS NULL',
        array(':msg_id' => $msg_id, ':u_id' => $u_id));
    if($query == "1")
    {
      $data_del = $this->db->update(PREFIX.'plugin_messages', array('date_read' => date('Y-m-d G:i:s')), array('id' => $msg_id));
      if($data_del > 0){
        return true;
      }else{
        return false;
  ...
  public function deleteMessageOutbox($u_id, $msg_id){
    ...
    $query = $this->db->select('SELECT * FROM '.PREFIX.'plugin_messages WHERE id = :msg_id AND from_userID = :u_id',
        array(':msg_id' => $msg_id, ':u_id' => $u_id));
    $count = count($query);
    if($count == "1")
    {
      $data_del = $this->db->update(PREFIX.'plugin_messages', array('from_delete' => 'true'), array('id' => $msg_id));
      if($data_del > 0){
        return true;
      }else{
        return false;
  ...
  public function cleanUpMessages(){
    $this->db->delete_open(PREFIX."plugin_messages WHERE to_delete = 'true' AND from_delete = 'true'");
  }
