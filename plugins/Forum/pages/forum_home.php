<?php
/**
* UserCandy Forum View Plugin Home
*
* UserCandy - Forum Plugin
* @author David (DaVaR) Sargent <davar@usercandy.com>
* @version uc 1.0.3
*/

use Helpers\{Csrf,CurrentUserData,TimeDiff};

/** Forum Home Page View **/
// Collect Data for view
  $data['title'] = $forum_title;
  $data['welcome_message'] = $forum_description;
  $data['site_description'] = $forum_description;

  // Get list of all forum categories
  $data['forum_categories'] = $model->forum_categories();

  // Get list of all forum categories
  $data['forum_titles'] = $model->forum_titles();
  ...
  // Setup Breadcrumbs
  $data['breadcrumbs'] = "<li class='breadcrumb-item active'>".$forum_title."</li>";
        ...
        foreach($data['forum_categories'] as $row)
      	{
              ...
              echo "$f_title";
                ...
                if($f_title == $row2->forum_title){
                        ...
                        echo "<a href='".SITE_URL."Forum/Topics/$f_id2/' title='$f_cat' ALT='$f_cat'>$f_cat</a></h4>";
                        echo "<div class='' style='text-align: left; font-size: x-small'>";
                            if($frp = ForumStats::forum_recent_posts("1", $f_id2)){
                                ...
                                echo " <a href='".SITE_URL."Forum/Topic/$frp_topic_id' style='font-weight: bold'>Last Post</a> ";
                                echo " by <a href='".SITE_URL."Profile/$frp_user_id' style='font-weight: bold'>$frp_user_name</a> " . TimeDiff::dateDiff("now", "$frp_tstamp", 1) . " ago ";
                            }
                        echo "</div>";
    							echo "</div>";
